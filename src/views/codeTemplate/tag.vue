  ...
  <a-card :bordered="false">
    <div>
      <div class="table-operator">
        <a-button type="primary" icon="plus" @click="$refs.addoredittag.add()">新建</a-button>
      </div>
      <a-table :columns="columns" :rowKey="record => record.id" :dataSource="data" :pagination="pagination" :loading="loading" @change="fanye">
        ...
        <span slot="id" slot-scope="text">
          <template>
            <a @click="$refs.addoredittag.edit(text)">编辑</a>
  ...
  methods: {
    ...
    getTemplate(index) {
      ...
      ajax.postSuccessCall(
        'api/CodeTemplateSetting/GetTagList',
        {
          pageIndex: index,
          pageSize: 20,
    ...
    deleteRow(id) {
      var that = this
      ajax.postSuccessCall(
        'api/CodeTemplateSetting/DeleteTag',
        {
          id: id
        },
        function(response) {
