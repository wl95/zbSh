        ...
        <a-table :columns="columns" :rowKey="record => record.id" :dataSource="data" :pagination="pagination" :loading="loading" @change="fanye">
          <span slot="id" slot-scope="text">
            <template>
              ...
              <el-button type="primary" size="mini" @click="distribution(text)" round>分配人员</el-button>
              <a-divider type="vertical" />
...
export default {
  ...
  methods: {
    ...
    add() {
      this.$router.push({ path: '/maintenance/projectModel', query: { id: 0 } })
    },
    edit(id) {
      this.$router.push({ path: '/maintenance/projectModel', query: { id: id } })
    },
    distribution(id) {
      this.$router.push({ path: '/maintenance/projectTouser', query: { id: id } })
    },
    deleteRow(id) {
      var that = this
      that
        ...
        .then(() => {
          ajax.get('api/Project/Delete', { id: id }).then(res => {
            if (res.result) {
