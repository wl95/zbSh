<template>
  <page-view title="部门列表">
    <a-card :bordered="false">
      <div>
        <div class="table-operator">
          <a-input-search placeholder="请输入部门名称" style="margin-left: 16px; width: 272px;" @search="onSearch" />
...
export default {
  ...
  methods: {
    ...
    add() {
      this.$router.push({ path: '/maintenance/departmentModel', query: { id: 0 } })
    },
    edit(id) {
      this.$router.push({ path: '/maintenance/departmentModel', query: { id: id } })
    },
    deleteRow(id) {
      var that = this
      that
        ...
        .then(() => {
          ajax.get('api/Department/Delete', { id: id }).then(res => {
            if (res.result) {
