<template>
  ...
  <div class="wl-explorer">
    ...
    <el-form class="wl-header-btn" :inline="true" :size="size" @submit.native.prevent>
      ...
      <el-form-item>
        <el-dropdown trigger="click" placement="bottom" @command="handleDropdown">
          <el-button style="color: #fff;" type="primary" plain>
            更多操作
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <!-- props自定义头部更多操作 -->
            <el-dropdown-item v-for="i of selfHeaderDropdown" :key="i.id" :icon="i.icon" :command="i.command" :divided="i.divided" :disabled="i.disabled">{{i.name}}</el-dropdown-item>
            <!-- solt自定义头部更多操作 -->
    ...
    <!--文件路径操作区-->
    <el-form :inline="true" :size="size" :model="file" class="wl-header-file" @submit.native.prevent>
      <el-form-item class="file-path-box">
        <!-- <div class="file-path-text" :class="{small: size=='small'}" v-show="!layout.edit_path" @click="handleFilePath">
          <img class="file-path-img" src="./images/folder@3x.png" alt="文件夹" title="文件夹" />
          {{file.path}}
        </div> -->
        <wlTreeSelect class="u-full" :size="size" :data="tree_path" :props="selfMoveProps" :nodeKey="selfProps.pathId" @change="filePathChange" v-model="searchTreeId"></wlTreeSelect>
        <!-- <el-autocomplete class="u-full" ref="file-path-ipt" placeholder="请输入文件路径" v-model="file.path" v-if="layout.edit_path" @keyup.enter.native="filePathChange" @select="filePathChange" :fetch-suggestions="pathQuerySearch">
      ...
      <el-form-item class="file-handle-box">
        <i class="iconfont icon-wl-left file-path-handle" :class="{'u-disabled':pathIsStart}" @click="pathBtn('prv')"></i>
        <i class="iconfont icon-wl-right file-path-handle" :class="{'u-disabled':pathIsEnd}" @click="pathBtn('next')"></i>
        <i class="iconfont icon-wl-up file-path-handle" :class="{'u-disabled':path.level===1}" @click="pathBtn('top')"></i>
      </el-form-item>
    </el-form>
    <!-- 主内容区 -->
    <el-scrollbar class="wl-main-scroll">
      ...
      <div class="wl-main-list">
        <!-- 表格型文件列表 -->
        <el-table v-show="layout.show_list" @selection-change="filrChecked" highlight-current-row :border="showBorder" :data="self_data" class="wl-table" ref="wl-table">
          ...
          <el-table-column v-for="i of selfColumns" show-overflow-tooltip :key="i._id" :prop="i.prop" :width="i.width" :label="i.label" :fixed="i.fixed" :align="i.align" :sort-by="i.sortBy" :sortable="i.sortable" :min-width="i.minWidth" :formatter="i.formatter" :column-key="i.columnKey" :class-name="i.className" :sort-method="i.sortMethod" :header-align="i.headerAlign" :render-header="i.renderHeader" :label-class-name="i.labelClassName">
        ...
        <ul class="wl-list" v-show="!layout.show_list">
          <li class="wl-list-item wl-is-folder" v-for="(i, idx) in self_data">
    ...
    <template v-if="usePreview">
      <file-view v-show="layout.view" ref="file-view" class="file-view-components" :previewType="previewType" :previewOptions="previewOptions" @close="layout.view = false"></file-view>
    </template>
    <!-- 移动文件区 -->
    <el-dialog title="移动文件" width="520px" :visible.sync="layout.move">
      <wlTreeSelect class="u-full" :size="size" :data="tree_path" :props="selfMoveProps" :nodeKey="selfProps.pathId" v-model="move_selected"></wlTreeSelect>
        ...
        <el-scrollbar class="scroll">
          <el-form :size="size" ref="template_form" label-position="top" class="template_form rule-form">
            <el-form-item label="文件路径">
              <wlTreeSelect class="u-full" :size="size" :data="tree_path" :props="selfMoveProps" :nodeKey="selfProps.pathId" v-model="upload_selected"></wlTreeSelect>
            </el-form-item>
            <el-form-item label="导入文件">
              <uploadItem ref="upload-item" :size="size" :reg="uploadReg" :url="uploadUrl" :limit="uploadLimit" :regFuc="uploadRegFuc" :options="uploadOptions" :headers="uploadHeaders" @beforeUpload="uploadBefore" @uploadSuccess="uploadSuccess" @uploadError="uploadError"></uploadItem>
            </el-form-item>
          </el-form>
        </el-scrollbar>
        <!-- 按钮区 -->
...
export default {
  ...
  methods: {
    ...
    filePathChange(item) {
      ...
      //   this.$confirm.alert(`文件管理器内找不到"${this.file.path}"。请检查拼写并重试。`, '文件资源管理器', {
      //     confirmButtonText: '确定',
      //     callback: () => { },
      ...
      this.$emit('search', file, this.file.key)
    ...
    fileSearch() {
      // if (this.file.key !== '') {
      var file = null
      if (this.searchTreeId !== null && this.searchTreeId.length > 0) {
        file = this.searchTreeId[0]
      }
      this.$emit('search', file, this.file.key)
      return
      // }
      // let _act_item = this.path.history.find(i => i.id === this.file.id)
      // _act_item ? this.routerActive(_act_item, _act_item.data) : this.$emit('search', this.file, true)
    },
    /**
     * 往历史里添加新的步骤
    ...
    // 文件上传提交操作
    saveUpload() {
      this.$emit('upload', this.upload_selected, this.handleUpload)
    },
    // 手动上传文件
    handleUpload() {
    ...
    fileTypeIcon(row) {
      ...
      if (row[this.selfIsFolder]) {
        _path = row[this.selfIsLock] ? require('./images/file_automatic@3x.png') : require('./images/folder@3x.png')
        return _path
      }
      // 其他根据后缀类型
